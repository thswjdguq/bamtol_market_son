...
**당근마켓 스타일 적용:**
- 홈 화면: 가로 스크롤 pill 버튼 스타일
- 검색 다이얼로그: 4열 아이콘 그리드 레이아웃

```dart
// 홈 화면 카테고리 (가로 스크```롤 pill)
ListView.builder(
  ...
  itemBuilder: (context, index) {
    return Obx(() {
      ...
      return GestureDetector(
        child: Container(
          decoration: BoxDecoration(
            color: isSelected ? Colors.white : Color(0xff3C3C3E),
        ...
        child: Column(
          children: [
            Container(
              ...
              height: 56,
              decoration: BoxDecoration(
                color: isSelected ? Color(0xffED7738) : Color(0xff3C3C3E),
              ),
              child: Icon(icon),
            ),
            AppFont(category),
...
**핵심 포인트:**
- GridView 전체를 Obx로 감싸면 안 됨 ❌
- 각 itemBuilder 내부에서 Obx 사용 ✅
...
✅ 10개 카테고리 추가 (전체, 전자기기, 의류, 가구, 도서, 스포츠/레저, 생활용품, 유아용품, 반려동물, 기타)
✅ 카테고리별 Material Icons 매핑
✅ 홈 화면 pill 스타일 카테고리 필터
✅ 검색 다이얼로그 아이콘 그리드 
✅ 검색어 입력 및 필터링
✅ 카테고리 + 검색어 조합 필터링
✅ 반응형 선택 상태 업데이트
✅ GetX 오류 해결 (Obx 위치 조정)
